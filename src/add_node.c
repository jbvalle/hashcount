...
void add_node(node_t **head, char *new_key){
    ...
    ///4. Check if first elements string member -> correct alphabetical order, else -> stack unto list
    ...
    ///5. Traverse through list using ptr and stop when correct slot for new element is found according to string member
    for(; (ptr->next != NULL)&&(strcmp(ptr->next->word, new_key) < 0); ptr = ptr->next);

    insert(head, ptr->word, new_key);

    ///6. If all preceding elements are preceding string members append new element to lost
